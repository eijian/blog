...
要するに文法定義が甘かったわけだ。2オペランド命令と1オペランド命令を
分けるだけでは個々の命令の細かい制限(Bレジスタしか指定できないとか)が
表現できていなかった。
...
(Applicativeへの対応はまた今度。上記の書き方だけでもまだ全然咀嚼できていない)
...
今回はアセンブラにアセンブリ言語を入れてマシンコードを生成させてCPUへ
入れたいと思う。アセンブラは生成したマシンコードを標準出力に出すので
...
ちゃんと動いている! やはりハンドアセンブルより断然楽々だ。

#### まとめ

さて、今回で本当に"CPU回"はおしまい。記事を細切れにしたせいもあって
10回にもなってしまった。長丁場だったが、CPUネタは面白い取り組みだった。
今後いろいろ拡張も考えよう。
