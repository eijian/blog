...
フィルタ $\boldsymbol{W}$ を修正する。大雑把には次の通り。

\[
\boldsymbol{W} \leftarrow \boldsymbol{W} - \eta \Delta \boldsymbol{W}
\]
...
この $\delta_i$ が次の層へ伝える「誤差」だ。
これらを踏まえ、各層の順伝播・逆伝播を表と図にしてみた
(逆伝播は、今回は全結合層のみ)。
なお、 $c$は教師データ。太字はベクトル/行列で、右下に要素数を記載した。
$\boldsymbol{W}_{[3,2]}$ は 3x2行列を表すこととする。
(層番号と要素数をつけたので非常に醜くなってしまった。。。)
...
|3|プーリング層1|$\boldsymbol{y}_{[5,5,10]}^{(3)} = \max (y_{(li+s)(lj+t)}^{(2)}), \; {\scriptsize where \: s,t\in |0,l|}$||
...
|6|プーリング層2|$\boldsymbol{y}_{[2,2,20]}^{(6)} = \max (y_{(li+s)(lj+t)}^{(5)}), \; {\scriptsize where \: s,t\in |0,l|}$||
...
|8|全結合層1|$\boldsymbol{a}_{[20]}^{(8)} = \boldsymbol{W}_{[80,20]}^{(8)} \cdot \boldsymbol{y}^{(7)} + \boldsymbol{b}_{[20]}^{(8)}$|$\boldsymbol{\delta}_{[20]}^{(8)} = f'(\boldsymbol{a}^{(8)}) \odot \boldsymbol{\delta}^{(9)}, \; {\scriptsize where \: f = \max}$|$\Delta w_{kl}^{(8)} = \delta_k^{(8)} \cdot y_l^{(7)},\; \boldsymbol{b}^{(8)} = \boldsymbol{\delta}^{(8)}$|
...
|10|全結合層2|$\boldsymbol{a}_{[3]}^{(10)} = \boldsymbol{W}_{[20,3]}^{(10)} \cdot \boldsymbol{y}^{(9)} + \boldsymbol{b}_{[3]}^{(10)}$|$\boldsymbol{\delta}_{[3]}^{(10)} = \boldsymbol{y}^{(11)} - \boldsymbol{c}_{[3]}$|$\Delta w_{ij}^{(10)} = \delta_i^{(10)} \cdot y_j^{(9)},\; \boldsymbol{b}^{(10)} = \boldsymbol{\delta}^{(10)}$|
|11|活性化層4(出力)|$\boldsymbol{y}_{[3]}^{(11)} = softmax (\boldsymbol{a}^{(10)})$|-|-|
