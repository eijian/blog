...
フォトンマップを使う以外は古典的な光線追跡のアルゴリズムを使おうと思う。
一番大枠の処理は次の通り。
...
3がメインとなるが今回は外堀を埋めよう。1、2、4を定義する。3も画素をループする
ところまで。

#### 仮想カメラの用意

1について必要なことを書き出してみる。細かい流儀などがあるだろうが、自分が
作るときに設定するのは以下のようなことだ。

* 視線
    - 視点(位置ベクトル)
    - 視線方向(方向ベクトル): スクリーンの中心方向
    - 垂直方向(方向ベクトル): スクリーンの「上」方向を決めるための情報
* スクリーン
...
これらの情報はちゃんとしたプログラムにするときは設定ファイルなどから読み込んで
いろいろ変更できるようにするのだろうが、とにかく「フォトンマッピング」法の効果を
見たいので決め打ち＝ソース中に直書きしてしまう。具体的には前回示したフォトン
マップの視覚化と同じ設定を使う。あとで比較できるし。今回使う設定は以下だ。

```haskell
eyepos = initPos 0 2 0  -- 視点
eyedir = ez3            -- 視線方向 (0, 0, 1)
upper  = ey3            -- 上方向を表すベクトル (0, 1, 0)  
focus  = 1.0 :: Double  -- フォーカス(スクリーンまでの距離)
xres   = 256 :: Int     -- 解像度(横方向)
yres   = 256 :: Int     -- 解像度(縦方向)
```

ただ、これだけでは実際に光線追跡するときに不十分なので、この情報を使っていくつかの
定義を追加で用意する。最初の画素の位置や次の画素の場所（縦横方向）などを決めて
やらないといけない。
...
```
point = origin + stepx * eex + stepy * eey
```
...
次のような形だったはずだ。フォトンキャッシュ情報は`PhotonCache`型である。
...
よって読み込みはこの逆をしてやればよい。ただし、最終的なデータ構造にはk-d treeを使う。
...
kdtで使うために読み込んだデータを変換して`PhotonInfo`型の`List`にしよう。
`PhotonInfo`型の定義、変換用関数は以下のとおり（`PhotonCache`型を使えば
いいのだが、フォトンの入射方向を逆転させる必要があるのでこれはあとで取り組む）。
また、k-d treeの構造に変換するときに必要になるので、フォトンの位置を取り出す
関数も定義しておく。
...
輝度値の書き出すため各周波数成分を画像ファイル用に整数の256段階に変換したい。
次にような関数を用意すれば良いだろう。
...
なお、'clip'は輝度値と最大値"255"(=rgbmax)の対応付けのため。0.1という設定は、
0.1[W]以上のエネルギーを持っていたら最大値とするということ。この値は適当なので、
出来上がる画像の明暗を見て適当に変更するつもりだ。`gamma'はガンマ補正のための
設定値で、一般的(?)な[tex: \gamma = 2.2] を用いる。

出力する画像データだが、一番簡単なPPM形式を用いよう。フォーマットはググればすぐ
わかるだろう。これを標準出力へ吐き出すようにする。以上を踏まえると、書き出し処理の
関数は次のようなものになるだろう。
...
各画素の輝度値のリストを引数にして書きだすだけだ。`convertOneCell'は一画素の
結果を文字列に直すもの。

#### まとめ

これまでの議論をまとめ、一つのソースにしてみた。
...
実行してみた結果は以下のとおり。当たり前だが面白くもなんともない・・・。
