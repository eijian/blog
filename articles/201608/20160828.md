...
まずはyusugomoriさんのPython実装(yusugomori実装)を写経するので、
そこで定義されているCNNを作りたいと思う。

### CNNの構造

前回出した図をもう一度使うが、CNNの構造は次のようになっている。
...
\[u_i = \sum_{j=0}^N x_j w_{ij} + b_i\]

ここで$x_j$はj番目の入力値、$u_i$はi番目の出力値、$b_i$はi番目用の
バイアス値である。
...
* 経過時間(秒)
* 正解率だ。
    - 正解率は、全テストデータについて出力リストの中から正解にあたる
      要素の値を取り出して平均したもの。
